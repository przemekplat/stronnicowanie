...
def calculate_elements_on_page(user_elements_count,user_page_count) #obliczanie ile elementów znajduje się na stronie
  ...
  additional_element_if_uneven = (elements-elements.to_i)*user_page_count #dodatkowe wiersze jeżeli liczba elementów jest niepodzielna przez ilość stron
...
def write_array(user_elements_count,user_page_count,elements_if_uneven,active_page,array)   #wpisywanie elementów do tablicy
  i = 0
  if elements_if_uneven > 0  #dodatkowe wiersze w przypadku gdy liczba elementow jest niepodzielna przez liczbę stron
    while i < elements_if_uneven
      array[i] << ""
      i += 1
    end
  end
...
elements_in_array, elements_if_uneven, active_page = calculate_elements_on_page(user_elements_count,user_page_count) #elements_in_array = elementy w tablicy, elements_if_uneven = dodatkowe elementy w tablicy gdy nierówne
...
write_array(user_elements_count,user_page_count,elements_if_uneven,active_page,array)
